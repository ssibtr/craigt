...
class UPSRequest():
    ...
    def set_package_detail(self, client, packages, packaging_type, namespace, ship_from, ship_to, cod_info, description):
        Packages = []
        for i, p in enumerate(packages):
            ...
            reference_number.Value = re.sub(r'\W+', ' ', description)
            ...
            if (p.name and ship_from.country_id.code in ('US') and ship_to.country_id.code in ('US')):
                reference_number2 = client.factory.create('ns3:ReferenceNumberType')
                reference_number2.Code = 'PM'
                reference_number2.Value = p.name
                reference_number2.BarCodeIndicator = p.name
                package.ReferenceNumber.append(reference_number2)
    ...
    def send_shipping(self, shipment_info, packages, shipper, ship_from, ship_to, packaging_type, service_type, saturday_delivery, cod_info=None, label_file_type='GIF', ups_carrier_account=False):
        ...
        for package in self.set_package_detail(client, packages, packaging_type, namespace, ship_from, ship_to, cod_info, shipment.Description):
            shipment.Package.append(package)
